...
extension Double4x4: Codable {
	public init(from decoder: Decoder) throws {
		...
		// 16 values, as this is a 4x4 matrix.
		//
	...
	public func encode(to encoder: Encoder) throws {
		...
		assert(values.count == 16, "Invalid number of values to encode")
		
		try values.encode(to: encoder)
	}
}

extension Double4x4: CustomStringConvertible {
	public var description: String {
		"""
		[[\(storage[0][0]), \(storage[1][0]), \(storage[2][0]), \(storage[3][0])],
		 [\(storage[0][1]), \(storage[1][1]), \(storage[2][1]), \(storage[3][1])],
		 [\(storage[0][2]), \(storage[1][2]), \(storage[2][2]), \(storage[3][2])],
		 [\(storage[0][3]), \(storage[1][3]), \(storage[2][3]), \(storage[3][3])]]
		"""
	}
}

extension Double4x4: Equatable {
...
extension Double4x4: Invertible {
...
extension Double4x4: Sendable {
	
}

extension Double4x4: SIMDConvertible {
	public typealias SIMDRepresentation = simd_double4x4
...
extension Double4x4: MatrixIdentity {
	public static var identity: Self {
		Self(matrix_identity_double4x4)
	}

	public var isIdentity: Bool {
		storage == matrix_identity_double4x4
	}
...
extension Double4x4: MatrixMath {
...
extension Double4x4: MatrixQuaternionConvertible {
	public typealias QuaternionRepresentation = DoubleQuaternion
...
extension Double4x4: MatrixRotation {
...
extension Double4x4: MatrixScale {
	public typealias Scale = Double3
...
extension Double4x4: MatrixTranslation {
	public typealias Translation = Double3
	
	public init(withTranslation translation: Translation) {
		var matrix = Self.identity
		for i in 0..<Self.rows {
			matrix.storage[3][i] = translation[i]
		}
		self = matrix
	}
...
extension Double4x4: MatrixTransposable {
...
extension Double4x4: MatrixVectorMath {
	public typealias VectorRepresentation = Double3
	
	public static func * (lhs: Self, rhs: VectorRepresentation) -> VectorRepresentation {
		var temp = simd_double4(rhs[0], rhs[1], rhs[2], .zero)
		temp = lhs.storage * temp
		return VectorRepresentation(temp[0], temp[1], temp[2])
	}

	public static func * (lhs: VectorRepresentation, rhs: Self) -> VectorRepresentation {
		var temp = simd_double4(lhs[0], lhs[1], lhs[2], .zero)
		temp = temp * rhs.storage
		return VectorRepresentation(temp[0], temp[1], temp[2])
	}
}
#else
...
//
	#warning("Double4x4 is not implemented on this platform.")
#endif

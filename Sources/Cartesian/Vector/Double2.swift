...
extension Double2: CustomStringConvertible {
...
extension Double2: Sendable {
	
}

extension Double2: SIMDConvertible {
	public typealias SIMDRepresentation = simd_double2
...
extension Double2: Vector2Rotatable {
	public func rotated(by angle: Angle<Component>) -> Self {
		let radians = angle.radians
			
		let u = self.first * cos(radians) - self.second * sin(radians)
		let v = self.first * sin(radians) + self.second * cos(radians)
...
extension Double2: VectorNormalizable {
...
extension Double2: VectorReflection {
...
//
	#warning("Double2 is not implemented on this platform.")
#endif

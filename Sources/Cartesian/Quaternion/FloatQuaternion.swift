...
extension FloatQuaternion: Blendable {
...
extension FloatQuaternion: Codable {
...
extension FloatQuaternion: Invertible {
...
extension FloatQuaternion: Quaternion {
	public typealias Component = Float
	public typealias Imaginary = Float3
	...
	public init<V>(from: V, to: V) where V : Vector3, Component == V.Component {
		self.storage = SIMDRepresentation(from: simd_float3(from[0], from[1], from[2]), to: simd_float3(to[0], to[1], to[2]))
	}
...
extension FloatQuaternion: QuaternionAxisAngle {
	public typealias AngleRepresentation = Float
	public typealias AxisRepresentation = Float3
...
extension FloatQuaternion: QuaternionMagnitude {
...
extension FloatQuaternion: QuaternionMath {
...
extension FloatQuaternion: QuaternionMatrixConvertible {
	public typealias MatrixRepresentation = Float3x3
...
extension FloatQuaternion: QuaternionNormalizable {
...
extension FloatQuaternion: QuaternionVectorRotation {
	public func rotate<V: Vector3>(vector: V) -> V where V.Component == Component {
		let vector = storage.act(simd_float3(vector.first, vector.second, vector.third))
		return V(vector[0], vector[1], vector[2])
	}
}

extension FloatQuaternion: Sendable {
	
}

extension FloatQuaternion: SIMDConvertible {
	public typealias SIMDRepresentation = simd_quatf
...
//
	#warning("FloatQuaternion is not implemented on this platform.")
#endif
